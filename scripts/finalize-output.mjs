...
const contentFolders = canonicalRoutes
  .filter(route => route !== '/explore/ev-guides')
  .map(route => route.split('/').at(-1));
const names = ['assets', 'scripts', ...contentFolders];
...
console.log(`Prepared self-contained dist: full root index, ${contentFolders.length} content folders, assets, scripts and child sitemap.`);
